...
function Chat() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [kbRes, modelRes, agentRes] = await Promise.all([knowledgeAPI.list(), modelAPI.list(), agentAPI.list()]);
  ...
  const isOnlyWelcomeMessage =
    messages.length === 0 || (messages.length === 1 && 'role' in messages[0] && messages[0].role === 'assistant');
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 200px)', gap: 0 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      {editingId === conv.id ? (
                        ...
                        <Text
                          ...
                          style={{
                            fontSize: 13,
                            color: conv.id === currentConversationId ? '#1677ff' : '#333',
      ...
      >
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          {isOnlyWelcomeMessage ? (
            ...
            >
              ...
              <Text style={{ fontSize: 24, fontWeight: 600, color: '#333', marginBottom: 8 }}>AI 智能助手</Text>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
                {['解释 React Hooks', '写一个 Python 爬虫', '推荐一本好书'].map((text) => (
                  <Button
                    ...
                    style={{ borderRadius: 20, padding: '8px 16px', height: 'auto', border: '1px solid #e8e8e8' }}
                  >
                    {text}
                  </Button>
                ))}
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      background: msg.role === 'user' ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : '#f7f7f8',
        ...
        {error && (
          ...
          >
            {error}
            <Button type="link" size="small" onClick={() => setError(null)} style={{ marginLeft: 8 }}>
              关闭
            </Button>
          </div>
        )}

        <div style={{ padding: '16px 24px 20px', borderTop: '1px solid #f0f0f0', background: '#fff' }}>
                        ...
                        <div style={{ marginTop: 16 }}>
                          ...
                          {(
                            [
                              ...
                              { value: 'function', label: 'Function Calling', desc: '自动选择工具调用' },
                          ...
                          {models.map((m) => (
                            ...
                            >
                              ...
                              <Text type="secondary" style={{ fontSize: 12 }}>
                                ...
                                {m.supports_vision ? '🖼️ 视觉 ' : ''}
                                {m.context_window ? `${(m.context_window / 1000).toFixed(0)}K 上下文` : ''}
                              </Text>
                            </div>
                          ))}
                        </div>
          ...
          <div
            style={{
              ...
              borderRadius: 12,
              padding: '8px 12px',
              border: isLoading ? '1px solid #667eea' : '1px solid transparent',
              ...
              {isLoading ? (
                ...
                <Button
                  ...
                  style={{
                    ...
                    height: 36,
                    borderRadius: 8,
                    background: inputValue.trim() ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : '#d9d9d9',
                    border: 'none',
                  }}
                />
              )}

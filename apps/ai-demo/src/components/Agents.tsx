...
function Agents() {
	...
	const [executeResult, setExecuteResult] =
		useState<AgentExecuteResponse | null>(null);
	...
	const columns = [
		...
		},
		{
			...
			key: "type",
			render: (type: string) => (
				<Tag color={AGENT_TYPE_COLORS[type] || "default"}>
						...
						children: (
							<Card>
								<Collapse accordion>
									...
									>
										<Text>
											ReAct (Reasoning + Acting) 是一种让 LLM
											交替进行推理和行动的模式。
					...
					>
						...
						{executeResult.steps && executeResult.steps.length > 0 && (
							<>
								...
								<Timeline
									style={{ marginTop: 12 }}
									items={executeResult.steps.map(
										(step: AgentStep, index: number) => ({
											children: (
												<div

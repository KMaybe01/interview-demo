...
function isValidBST(root) {
  ...
  function inorder(node) {
    ...
    if (prev !== undefined && node.val <= prev) return false;
    prev = node.val;
...
function decodeString(s) {
  ...
  for (const c of s) {
    if (c >= '0' && c <= '9') currNum = currNum * 10 + +c;
      ...
      strStack.push(currStr);   // 保存当前已累积的字符串前缀
      currNum = 0;
      currStr = '';             // 重置，开始收集内层内容
    }
    else if (c === ']') {
      ...
      currStr = prevStr + currStr.repeat(repeatCount);  // 恢复前缀 + 展开内层
    }
    else currStr += c;
  }
  return currStr;
...
**📝 通俗解释**：遇到 `[` 时先把当前累积的字符串保存到 strStack（而不是空串），遇到 `]` 时弹出前缀，将内层展开的内容拼接到前缀后面。这样嵌套括号也能正确处理，例如 `"3[a2[c]]"` → `"accaccacc"`。

**🔍 嵌套示例追踪**：`"3[a2[c]]"`

- `3[` → numStack=[3], strStack=[''], currStr=''
- `a` → currStr='a'
- `2[` → numStack=[3,2], strStack=['a',''], currStr=''（保存了 'a'）
- `c` → currStr='c'
- `]` → currStr = '' + 'c'.repeat(2) = 'cc'
- `]` → currStr = 'a' + 'cc'.repeat(3) = 'accaccacc' ✅
...
function calculate(s) {
  ...
  for (let i = 0; i <= s.length; i++) {
    ...
    if (c >= '0' && c <= '9') num = num * 10 + +c;
      ...
      if (i < s.length) { sign = c; }  // i === s.length 时不更新 sign（最后一次迭代）
      num = 0;
    }
  }
...
function mySqrt(x) {
  if (x < 2) return x;
  let l = 2, r = Math.floor(x / 2), ans = 2;
  while (l <= r) {
    const mid = Math.floor((l + r) / 2);
    if (mid <= Math.floor(x / mid)) { ans = mid; l = mid + 1; }
    else r = mid - 1;
  }
  return ans;
}

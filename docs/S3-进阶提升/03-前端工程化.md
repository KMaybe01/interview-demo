...
### 5️⃣ Dual-Package Hazard（双包危害）

> **定义**：在同一个应用中，由于依赖链的混乱，导致某个第三方库同时被加载了其 ESM 版本（.mjs）和 CommonJS 版本（.cjs），内存中存在两个独立实例。

**产生原因：** 当依赖树中出现两条不同的解析路径，一条解析到包的 ESM 入口，另一条解析到 CJS 入口，Node.js/bundler 不会将它们视为同一个模块，导致两份代码同时加载。

```mermaid
flowchart LR
    subgraph "正常：单实例"
        A["App (ESM)"] -->|"import 'pkg'"| B["pkg (ESM)"]
        C["Lib (ESM)"] -->|"import 'pkg'"| B
    end
    subgraph "双包危害：双实例"
        D["App (ESM)"] -->|"import 'pkg'"| E["pkg (ESM)"]
        F["Lib (CJS)"] -->|"require('pkg')"| G["pkg (CJS)"]
    end
```

**危害：**
- 如果该库拥有**内部全局状态**（如 Zustand store、Singleton 实例、全局缓存、模块级变量），双实例会导致状态无法同步
- 包体积翻倍增大
- 类型定义可能冲突，`instanceof` 检查失效

**典型场景：**

| 场景 | 引发原因 | 后果 |
|------|---------|------|
| Zustand store | 一个版本在 ESM 中 `create()`，另一个在 CJS 中 `create()` | 两份 store，组件看不到对方的更新 |
| 全局缓存模块 | 模块级 `Map` 或 `WeakMap` 在不同实例中独立 | 缓存穿透，重复计算 |
| 日志/监控库 | 初始化逻辑在两份实例中各执行一次 | 重复上报、日志混乱 |
| `instanceof` 类型判断 | 两个不同的 class 定义 | `obj instanceof MyClass` 永远为 `false` |

**Rolldown/Vite 8 的防御最佳实践：**

```javascript
// vite.config.ts — 强制锁定包解析路径
...
export default defineConfig({
  resolve: {
    alias: {
      // 强制整个应用使用同一份 pkg 实例
      'pkg': require.resolve('pkg'), // Node.js 实际解析路径
    },
    dedupe: ['zustand', 'react', 'react-dom'], // Vite 特有：强制去重
  },
})
```

**package.json 规范导出配置（库作者）：**
...
| 原则 | 说明 |
|------|------|
| **锁定版本** | 使用 `resolutions`（Yarn）/ `overrides`（npm）/ `pnpm.overrides` 统一依赖版本 |
| **去重解析** | Vite 的 `resolve.dedupe` 或 Webpack 的 `resolve.alias` 强制单路径 |
| **exports 优先** | 库作者优先使用 `exports` 字段，避免 `main` + `module` 歧义 |
| **检测手段** | `Object.keys(require.cache)` 排查重复实例，或运行时检查全局 `Symbol` 标记 |

```javascript
// 运行时检测双包危害（防御性代码）
const PKG_KEY = Symbol.for('my-lib:instance')
if (globalThis[PKG_KEY]) {
  console.warn('[my-lib] 检测到多实例加载，请检查 resolve.alias 配置')
} else {
  globalThis[PKG_KEY] = true
}
```

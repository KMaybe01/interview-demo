...
| **百万行日志流式解密** | 生产/消费模式 + AES-256-GCM 解密 + RSA 密钥交换 + 虚拟滚动 | ⭐⭐ |
...
| **兜底** | Refresh 过期 → 清 Token → 登出 | 不循环重试，防止死循环 |
...
```
当前实现是演示页面，核心区别:
  1. 触发方式: 手动点击按钮模拟，而非自动拦截所有 fetch
  2. 拦截范围: simulateRequest 手动构造请求，而非全局 fetch 封装
  3. 生产方案建议: 封装统一的 request/fetch 函数，内部自动处理 401:
     async function request(url, options) {
       const res = await fetch(url, { ...options, headers: { Authorization: `Bearer ${getAccessToken()}` } })
       if (res.status === 401) {
...
graph TB
    ...
    subgraph UI["UI 展示层（RbacPermission.tsx）"]
        ...
        ROUTE["路由层<br/>Table 展示路由访问控制"]
        BTN["按钮层<br/>Table 展示按钮启用/禁用"]
    end

    DEF --> UI
```

> 注意：此页面为**纯前端演示**，展示位编码权限的核心概念和三层联动效果。实际生产环境需配合后端 API 做二次鉴权。
...
页面使用 Ant Design Tree + Table 组件可视化展示三层联动效果:
  ...
  2. 路由层: Table 展示路由级权限检查，显示 granted/denied 权限对比
  3. 按钮层: Table 展示按钮级权限，逐项检查 hasPermission

注意: 这是演示页面，不是真实的路由守卫。真实场景需要服务端配合做二次鉴权。
```
...
**Q4: 权限码枚举为什么用常量对象而非 TypeScript enum？**
```
...
```
当前实现为纯前端演示，核心价值在于展示位运算编码原理。
生产环境需要:
  a. 后端 API 二次鉴权（前端控制 UI，后端控制数据）
  b. 动态角色/权限管理后台
...
**第四步：两阶段 useEffect — 初始加载 vs 过期检测**

每个页面组件有两个 `useEffect`，职责分离：

```typescript
// 第一阶段：初始加载（组件挂载时执行一次）
const dataLoadedRef = useRef(false)
useEffect(() => {
  if (dataLoadedRef.current && page.data) return  // 仅首次
  dataLoadedRef.current = true
  setLoading(pageKey, true)
  fetchData().then(() => { updateData(pageKey, data); clearStale(pageKey) })
}, [pageKey, page.data, fetchData])

// 第二阶段：过期检测（仅在 stale 或 TTL 过期时触发）
...
}, [isActive, isStale, pageKey, page.loadedAt, fetchData, clearStale])
```

关键设计：
| Ref | 作用 | 生命周期 |
|-----|------|----------|
| `dataLoadedRef` | 防止初始加载重复执行 | 组件挂载期间持久 |
| 无 activeRef | `wasActive` 不再需要 — 第二段 effect 仅根据 `isStale`（写后失效标记）或 `isTtlExpired`（30s 惰性过期）触发刷新，激活切换本身不会引发请求 | — |

> ⚠️ **踩坑记录**：早期版本使用 `(activeRef.current || isStale) && isActive` 条件判断，导致死循环。随后改用 `wasActive` 模式修复（追踪 inactive→active 切换）。最终简化为**移除 activeRef 完全**，第二段 effect 不再响应激活状态切换，仅响应 `staleKeys` 变化和 TTL 过期。三个条件合一：`isStale || isTtlExpired`。激活切换但不满足这两个条件时，页面直接从缓存读取数据，不发起网络请求。
...
| **外部数据变更无法感知**（如返回列表页时数据已被其他用户修改） | 三种可选方案：① **惰性过期（已实现）** — 切回时判断 `Date.now() - loadedAt > 30s` 自动刷新，在 MonitorPage/LogsPage 第二段 useEffect 中加入 TTL 检查；② **WebSocket 推送** — 服务端主动推送 `invalidate` 事件（待实现）；③ **Pull-to-refresh** — 用户主动点击刷新按钮 |

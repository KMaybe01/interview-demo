...
| **LRU 路由缓存** | DOM display:none 保持状态 + LRU 淘汰 + staleKeys 写后失效 + 两阶段 useEffect + 惰性刷新 | ⭐⭐ |
...
// 第二阶段：过期检测（每次激活或 stale 变化时触发）
const activeRef = useRef(false)
const isStale = staleKeys.includes(pageKey)
useEffect(() => {
  // activeRef.current: 首次激活后为 true，后续切回都刷新
  // isStale: 其他页面 invalidateAll 标记为过期，强制刷新
...
| `activeRef` | 记录是否已被激活过，后续每次切回都刷新 | 跟随组件挂载 |
...
| 如何确保每次切回都刷新 | activeRef 标记是否已被激活过 |

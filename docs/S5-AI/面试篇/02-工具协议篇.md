...
### Q17: 当有大量工具时，如何让 LLM 准确选择正确的工具？

> 💡 **要点**：工具多不是问题，关键是别让模型在一个没有层次的工具海里盲选

当注册的工具超过 20 个时，LLM 选择准确率显著下降。以下是治理策略：

| 策略 | 原理 | 适用场景 |
|------|------|---------|
| **工具分层** | 先选类别再选具体工具 | 工具数量 > 20 个 |
| **动态工具选择** | 基于用户意图预筛选 | 特定场景只暴露相关工具 |
| **工具聚合** | 将多个工具合并为单一高阶工具 | 相关操作类工具（CRUD） |
| **描述优化** | 工具名 + 描述自解释 | 所有场景 |
| **Few-shot 示例** | 给出典型调用样例 | 模型对工具不熟悉时 |

```
# 工具分层架构示例
L1: 意图分类 → L2: 工具类别 → L3: 具体工具

用户输入 "帮我搜索一下最新的AI论文"
  → 意图分类: "搜索"
    → 工具类别: "知识检索工具"
      → 具体工具: web_search(query, limit=10)
```

**核心原则：**
1. **分类治理**：超过 20 个工具必须分层组织
2. **名称即文档**：工具名和参数名要自描述（`search_web_by_keyword` 而非 `search1`）
3. **参数扁平化**：避免深层嵌套 JSON Schema
4. **动态注册**：根据上下文只暴露当前任务需要的工具子集

---

### Q18: MCP 如何实现并发控制和请求管理？

> 💡 **要点**：MCP 通过唯一请求 ID、并行处理、限流机制实现并发控制

MCP 基于 JSON-RPC 2.0，天然支持请求-响应的关联。以下是并发控制的关键机制：

```mermaid
graph TB
    subgraph "MCP 请求管理"
        C1["客户端发起请求"] --> ID["生成唯一 requestId"]
        ID --> Queue["请求队列"]
        Queue --> Parallel["并行处理池"]
        Parallel --> Server1["MCP Server A"]
        Parallel --> Server2["MCP Server B"]
        Parallel --> Server3["MCP Server C"]
        Server1 --> Response["响应匹配 requestId"]
        Server2 --> Response
        Server3 --> Response
    end
```
...
| 机制 | 说明 | 实现方式 |
|------|------|---------|
| **唯一请求 ID** | JSON-RPC 2.0 标准，每个请求携带 `id` 字段 | `{"jsonrpc":"2.0","id":"req-001","method":"tools/call"}` |
| **并行请求** | 客户端可同时发送多个请求，不等待前一个完成 | 基于 HTTP/2 或独立 TCP 连接 |
| **请求限流** | 服务端通过 `rate_limit` 参数控制请求速率 | 返回错误码 `-32000`（Rate Limited） |
| **优先级队列** | 重要请求优先处理 | 请求头部携带 `priority` 字段 |
| **超时控制** | 请求在规定时间内未完成则取消 | 客户端设置 `timeout` 参数 |
| **批量操作** | 多个工具调用合并为一次请求 | JSON-RPC 批量请求（`[]` 数组格式） |

**Streamable HTTP 下的并发：**
- 支持多个 `tools/call` 请求同时发送
- 每个请求有独立 `requestId`，响应通过 `id` 字段匹配
- 服务端可选择串行或并行处理，通过 `parallel` 能力声明告知客户端
- 2025+ 生产环境推荐使用 Streamable HTTP + 请求 ID 模式

---

### Q19: 如何管理 Prompt 的版本？

> 💡 **要点**：Prompt 版本管理不是单纯存文本，而是要同时管理模板、参数、评估集和发布状态

**Prompt 版本管理的核心要素：**

| 要素 | 说明 | 工具支持 |
|------|------|---------|
| **内容版本化** | 每次修改生成新版本，可回滚 | Git + 语义版本 |
| **参数模板化** | 变量占位符与值分离 | `{variable}` 模板语法 |
| **A/B 测试** | 同一场景多版本并行 | 按用户/流量百分比分配 |
| **评估集关联** | 每个版本关联测试用例 | LLM as Judge 评分 |
| **发布管理** | 草稿/测试/生产环境隔离 | 审批流 + 自动部署 |

**版本管理流程：**

```
开发环境 → 测试环境（A/B 测试）→ 生产环境（灰度发布）

v1.0 → v1.1（修复注入漏洞）→ v1.2（优化格式）
                            ↘ v2.0（改用新范式）
```

**最佳实践：**
1. **版本号**：遵循语义化版本（SemVer）— 主版本（大改）、次版本（增删）、补丁（修漏洞）
2. **变更日志**：每次修改标注变更原因、影响范围、测试结果
3. **自动测试**：每次版本变更自动在测试集上跑分，分数下降自动回滚
4. **灰度发布**：先 10% 流量 → 观察指标 → 逐步放量到 100%
5. **Prompt 即代码**：将 Prompt 纳入 CI/CD 流程，像管理代码一样管理 Prompt
